...
func (_q *PlanQuery) QueryUser() *UserQuery {
...
func (_q *PlanQuery) QueryDailyTasks() *DailyTaskQuery {
	query := (&DailyTaskClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PlanQuery) Clone() *PlanQuery {
	...
	return &PlanQuery{
		...
		withDailyTasks: _q.withDailyTasks.Clone(),
...
func (_q *PlanQuery) WithUser(opts ...func(*UserQuery)) *PlanQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *PlanQuery) WithDailyTasks(opts ...func(*DailyTaskQuery)) *PlanQuery {
	query := (&DailyTaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDailyTasks = query
	return _q
}
...
func (_q *PlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plan, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Plan, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDailyTasks; query != nil {
		if err := _q.loadDailyTasks(ctx, query, nodes,
			func(n *Plan) { n.Edges.DailyTasks = []*DailyTask{} },
			func(n *Plan, e *DailyTask) { n.Edges.DailyTasks = append(n.Edges.DailyTasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PlanQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *User)) error {
...
func (_q *PlanQuery) loadDailyTasks(ctx context.Context, query *DailyTaskQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *DailyTask)) error {
	...
	query.Where(predicate.DailyTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plan.DailyTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.plan_daily_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "plan_daily_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "plan_daily_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
