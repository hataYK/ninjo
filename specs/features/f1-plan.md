...
妥当性をレビューしてフィードバックする。
計画を達成するとアバターにスキルが蓄積され、分身の成長につながる。

## ユーザーストーリー

> ユーザーとして、勉強したい本と期限を登録したら、
> AIが「そのペースで現実的か」を教えてくれて、安心して始めたい。
> この計画をやり切れば、アバターに新しいスキルが身につくのが楽しみ。
...
計画を削除する。紐づくデイリータスクも削除。スキルは保持される（学びは消えない）。
...
- 進捗（current_page相当）は daily_tasks の actual_end_page から導出する（plans テーブルには持たない）
- progress_rate = 導出した現在ページ / total_pages
- 全ページ完了で status を completed に自動変更
- 可処分時間が未設定（全曜日0h）でもレビュー可能。その場合はtotal_daysで割る
- 計画を削除してもスキルは保持される（原則6: 学びは消えない）
